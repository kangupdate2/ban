...
class MirrorStatus:
    ...
    STATUS_PAUSE = "⛔️ Menghentikan..."
    STATUS_ARCHIVING = "🔐 MengArsip..."
    STATUS_EXTRACTING = "📂 MengEkstrak..."
    STATUS_SPLITTING = "✂️ MengEkstrak..."
...
def get_readable_message():
        ...
        for index, download in enumerate(list(download_dict.values())[COUNT:], start=1):
            ...
            if download.status() not in [
                ...
                if download.status() == MirrorStatus.STATUS_CLONING:
                    msg += f"\n<b>↳🚦TerSalin:</b> {get_readable_file_size(download.processed_bytes())} of {download.size()}"
 ...
 #       bmsg += f"\n<b>RAM:</b> {virtual_memory().percent}% | <b>UPTIME:</b> {get_readable_time(time() - botStartTime)}"
        ...
        if STATUS_LIMIT is not None and tasks > STATUS_LIMIT:
            buttons = ButtonMaker()
            buttons.sbutton("Sebelum", "status pre")
            buttons.sbutton(f"{PAGE_NO}/{pages}", str(THREE))
            buttons.sbutton("Berikut", "status nex")
            buttons.sbutton("STATUS", str(FOUR))
