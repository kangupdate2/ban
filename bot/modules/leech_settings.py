...
def setLeechType(update, context):
    ...
    if user_id != int(data[1]):
        query.answer(text="Siapa Kamu!", show_alert=True)
    elif data[2] == "doc":
        ...
        query.answer(text="File TELE berbentuk ke Document!", show_alert=True)
        ...
        query.answer(text="File TELE berbentuk ke Media!", show_alert=True)
        editLeechType(message, query)
    elif data[2] == "thumb":
...
def setThumb(update, context):
    ...
    if reply_to is not None and reply_to.photo:
        ...
        sendMessage("Reply dengan sebuah foto untuk menyimpan custom thumbnail.", context.bot, update.message)
