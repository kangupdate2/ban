...
from bot import bot, Interval, INDEX_URL, BUTTON_FOUR_NAME, BUTTON_FOUR_URL, BUTTON_FIVE_NAME, BUTTON_FIVE_URL, \
                ...
                LEECH_LOG, SOURCE_LINK, BOT_PM, MIRROR_LOGS, AUTO_DELETE_UPLOAD_MESSAGE_DURATION, RSS_CHAT_ID, FSUB
...
def _mirror(bot, message, isZip=False, extract=False, isQbit=False, isLeech=False, pswd=None, multi=0):
    ...
    if not is_url(link) and not is_magnet(link) and not ospath.exists(link):
        ...
        help_msg += f"\n/{BotCommands.MirrorCommand} [link] |NamaBaru pswd: xx [zip/unzip]"
        help_msg += f"\n\n<b>Dengan balas link atau file</b>"
        help_msg += f"\n/{BotCommands.MirrorCommand} |NamaBaru pswd: xx [zip/unzip]"
